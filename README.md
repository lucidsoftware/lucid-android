...
The `@LifecycleManaged` annotation helps address this by allowing you to annotate top-level members with the lifecycle method they should be initialized in. A new `LifecycleValue` typed is introduced to help with debugging and provide some guarantees around access lifecycle managed members. The above example becomes:
